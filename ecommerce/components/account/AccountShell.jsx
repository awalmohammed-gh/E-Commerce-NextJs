...
export function AccountNav() {
  ...
  return (
    <nav aria-label="Account sections" className="-mx-4 border-b border-line sm:-mx-6 lg:hidden">
      <ul className="flex gap-6 overflow-x-auto px-4 no-scrollbar sm:px-6">
          ...
          return (
            <li key={link.href} className="shrink-0">
              <Link
                ...
                className={`relative flex min-h-12 items-center text-[14px] whitespace-nowrap transition-colors ${
                  active ? "text-ink after:absolute after:inset-x-0 after:-bottom-px after:h-0.5 after:bg-ink" : "text-muted hover:text-ink"
                }`}
              >
                {link.short}
              </Link>
    ...
    <aside className="hidden lg:block">
      <div className="sticky top-28">
        <div className="flex items-center gap-3 border-b border-line pb-6">
          {user ? <Avatar user={user} /> : <span className="skeleton h-10 w-10 rounded-full" />}
          <div className="min-w-0">
            {user ? (
              <>
                <p className="truncate text-[15px] font-medium text-ink">{user.fullName}</p>
              ...
              return (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    aria-current={active ? "page" : undefined}
                    className={`flex min-h-11 items-center gap-3 rounded-[3px] px-3 text-[15px] transition-colors ${
                      active ? "bg-cream text-ink" : "text-ink-soft hover:bg-cream/70 hover:text-ink"
                    }`}
                  >
                    <Icon className={`h-4 w-4 ${active ? "text-ink" : "text-taupe"}`} strokeWidth={1.6} aria-hidden="true" />
                    {link.label}
                  </Link>
                </li>
              );
        ...
        <button
          type="button"
          onClick={signOut}
          className="flex min-h-11 w-full items-center gap-3 border-t border-line px-3 pt-4 text-[15px] text-ink-soft transition-colors hover:text-danger"
        >
