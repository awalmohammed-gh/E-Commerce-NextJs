...
export default function MyCheckout() {
  ...
  /* Navigate to the create-address page */
  const goToCreateAddress = () => router.push("/create-address");

  /* Navigate to edit an existing address */
  const goToEditAddress = (addr) =>
    router.push(`/create-address?id=${addr.id}`);
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0 text-sm">
                        ...
                        <p
                          className={`text-xs ${
                            isSelected ? "text-[#F5F1EA]/60" : "text-[#8A6A52]"
                          }`}
                        >
                          {method.description}
                        </p>
              ...
              {Object.entries(addItems).map(([productId, sizes]) =>
                Object.entries(sizes).map(([size, qty]) => {
                  ...
                  return (
                    ...
                    >
                      <p className="text-[#1C1A17] font-medium">{productId}</p>
